...
public class Game {
	...
	public boolean isGameOver() {
		if (reserves[colorToPlay].size() > 0) {
			return false;
		}
		for (int r = 0; r < BOARD_WIDTH; r++) {
			for (int c = 0; c < BOARD_WIDTH; c++) {
	...
	public boolean isLegalSource(Location source) {
		for (int color = BLACK; color <= WHITE; color++) {
			if (source == RESERVES_LOCATIONS[color]) {
				return reserves[color].size() > 0 && colorToPlay == color;
			}
		}
	...
	public void move(Location source, Location destination) {
		...
		while (d.size() > 5) {
			...
			if (color == getColorToPlay()) {
				reserves[colorToPlay].addBack(colorToPlay);
			}
		}
	}
